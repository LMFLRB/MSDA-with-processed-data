...
def mmd(*features, params=[1,2,5,10]):
   res=[mix_rbf_mmd2(features[row],features[col],params) for row,col in zip(*np.triu_indices(len(features),1))]
   return sum(res) if len(res)>1 else res[0]
